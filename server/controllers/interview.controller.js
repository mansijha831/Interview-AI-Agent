...
export const analyzeResume = async (req, res) => {
  try {
    ...
    // yaha se upar wale line ko comment karke kiya h ye likha h uske badle
    let parsed;
    try {
...
`,
    ...
    console.log("QUESTIONS FIELD:", interview.questions);
    res.json({
...
export const submitAnswer = async (req, res) => {
  try {
    ...
    if (timeTaken > question.timeLimit) {
      question.score = 0;
      question.feedback = "Time limit exceede.Answer not evaluated.";
...
export const getMyInterviews = async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: `failed to find currentUser
      Interview ${error}`,
    });
  }
};
...
export const getInterviewReport = async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: `failed to find currentUser Interview report
      ${error}`,
    });
  }
};
